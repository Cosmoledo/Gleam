...
export const loadCanvas = (url: string): Promise<HTMLCanvasElement> => {
	return new Promise((resolve): void => {
		const image = new Image();
		image.onload = (): void => {
...
export const loadText = (url: string): Promise<string> => {
	return fetch(url).then((raw: Response) => raw.text());
};

export const loadJson = (url: string): Promise<any> => {
	return fetch(url).then((raw: Response) => raw.json());
};

export const loadJsonCommented = (url: string): Promise<any> => {
	return fetch(url)
		.then((raw: Response) => raw.text())
		.then((text: string) => {
...
export const loadImageFromJson = async (
	baseUrl: string,
	nameOrJson: string,
	jsonInput = false,
): Promise<any> => {
	...
	const json = jsonInput
		? nameOrJson
		: await loadJson(baseUrl + nameOrJson + ".json");

	return loadCanvas(baseUrl + json.options.file).then(
		(canvas: HTMLCanvasElement) => {
			const sprites: any = {};

			json.sprites.forEach((sprite: any) => {
